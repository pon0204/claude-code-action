...
jobs:
  claude-response:
    ...
    steps:
      - uses: pon0204/claude-code-action@beta  # Fork with OAuth support
        with:
...
steps:
  - uses: pon0204/claude-code-action@beta
    with:
      direct_prompt: |
        Update the API documentation in README.md to reflect
        the changes made to the API endpoints in this PR.
...
This action is built on top of [`pon0204/claude-code-base-action`](https://github.com/pon0204/claude-code-base-action).
